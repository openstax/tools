...
def incl(o1, o2)
  ...
  when Hash
    {}.tap do |hh|
      oo2.each do |key, value|
        result = incl(oo1[key], value) rescue value
  ...
  when Array
    ...
    oo2.each_with_index.map do |value, ii|
      if match_any
        oo1.map do |oo1v|
          incl(oo1v, value) rescue value
        end.select(&:blank?).first
      else
        incl(oo1[ii], value) rescue value
      end
    end.reject(&:blank?)
  else
    (o1.is_a?(RSpec::Matchers::BuiltIn::BaseMatcher) ? o1.matches?(o2) : o1 == o2) ? nil : o2
  end
end
